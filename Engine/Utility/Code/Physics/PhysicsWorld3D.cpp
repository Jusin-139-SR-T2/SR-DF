...
_uint CPhysicsWorld3D::Generate_Contacts()
{
	...
	for (auto iter = m_listBody.begin(); iter != m_listBody.end(); ++iter)
	{
		...
		FCollisionPrimitive* pColSrc = static_cast<FCollisionPrimitive*>((*iter)->Get_Owner());
...
void CPhysicsWorld3D::Add_RigidBody(FRigidBody* pBody)
{
	...
	reinterpret_cast<FCollisionPrimitive*>(pBody->Get_Owner())->Calculate_Shape();
	m_pBVHRootNode->Insert(pBody, reinterpret_cast<FCollisionPrimitive*>(pBody->Get_Owner())->BoundingBox);
}
