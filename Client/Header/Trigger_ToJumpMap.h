...
class CTrigger_ToJumpMap : public CTrigger
{
	DERIVED_CLASS(CTrigger, CTrigger_ToJumpMap)

protected:
	explicit CTrigger_ToJumpMap(LPDIRECT3DDEVICE9 pGraphicDev);
	explicit CTrigger_ToJumpMap(const CTrigger_ToJumpMap& rhs);
	virtual ~CTrigger_ToJumpMap();

public:
	static CTrigger_ToJumpMap* Create(LPDIRECT3DDEVICE9 pGraphicDev, const FSerialize_GameObject& tObjectSerial);
